...
async def product(product_id: int, response: Response, db: Session = Depends(get_db)):
    ...
    if not product:
        raise HTTPException(status_code= status.HTTP_404_NOT_FOUND, detail= f"Product with the id {product_id} is not available")
    return product
...
async def products(provider: int=None, type: str=None, skip: int=0, limit: int=100, db: Session = Depends(get_db)):
    if provider and type:
        products = db.query(models.Product).filter(models.Product.provider_id == provider, models.Product.type == type).all()
    elif provider:
        products = db.query(models.Product).filter(models.Product.provider_id == provider).all()
    elif type:
        products = db.query(models.Product).filter(models.Product.type == type).all()
    else:
...
async def product(product: schemas.Product, response: Response, db: Session = Depends(get_db)):
    ...
    if not provider:
        raise HTTPException(status_code= status.HTTP_404_NOT_FOUND, detail= f"Provider with the id {product.provider_id} is not available")
...
async def product(provider_id: int, response: Response, db: Session = Depends(get_db)):
    product = db.query(models.Provider).filter(models.Provider.id == provider_id).first()
    if not product:
        raise HTTPException(status_code= status.HTTP_404_NOT_FOUND, detail= f"Provider with the id {provider_id} is not available")
    return product
...
async def provider(provider_id: int, provider: schemas.UpdateProvider, db: Session = Depends(get_db)):
    update_provider = db.query(models.Provider).filter(models.Provider.id == provider_id)
    if not update_provider.first():
        raise HTTPException(status_code= status.HTTP_404_NOT_FOUND, detail= f"Product with the id {provider_id} is not available")
...
async def product(provider_id: int, db: Session = Depends(get_db)):
    provider = db.query(models.Provider).filter(models.Provider.id == provider_id)
    if not provider.first():
        raise HTTPException(status_code= status.HTTP_404_NOT_FOUND, detail= f"Product with the id {provider_id} is not available")
    product = db.query(models.Product).filter(models.Product.provider_id == provider_id)
    if product.first():
        raise HTTPException(status_code= status.HTTP_405_METHOD_NOT_ALLOWED, detail= f"Provider {provider_id} has associated products")
